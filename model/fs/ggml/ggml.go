...
func (kv KV) HeadCount() []uint64 {
	...
	if len(headCount) > nLayers {
		log.Warn("got more elements of attention.head_count than layers", "len(headCount)", len(headCount), "layers", nLayers)
	}
...
func (kv KV) EngineRequired() bool {
...
func (f GGML) GraphSize(context, batch uint64, numParallel int, kvCacheType string, useFlashAttention bool) (kv []uint64, partialOffload, fullOffload uint64) {
	...
	case "gemma", "gemma2", "gemma3", "gemma3n":
		...
		// Gemma2 also has sliding window attention but we only have an optimized implementation in the llama.go
		// engine. Gemma3 always uses the llama.go engine.
