...
// This is a higher level interface that maps harmony concepts into llama.go concepts
...
func (h *HarmonyMessageHandler) AddContent(content string, toolParser *HarmonyToolCallAccumulator) (string, string, string) {
	...
	for _, event := range events {
		...
		case HarmonyEventHeaderComplete:
			log.Trace("harmony event header complete", "header", event.Header)
		...
		case HarmonyEventContentEmitted:
			log.Trace("harmony event content", "content", event.Content, "state", h.state)
