...
func (m *IModel) Capabilities() []model.Capability {
	...
	if m.ModelPath != "" {
		...
		if err == nil {
			...
			log.Error("couldn't open model file", "error", err)
		}
		...
		log.Warn("unknown capabilities for model", "model", m.Name)
	}
...
func (m *IModel) CheckCapabilities(want ...model.Capability) error {
	...
	for _, cap := range want {
		err, ok := capToErr[cap]
		if !ok {
			log.Error("unknown capability", "capability", cap)
...
func GetModel(name string) (*IModel, error) {
...
func deleteUnusedLayers(deleteMap map[string]struct{}) error {
	...
	for k := range deleteMap {
		...
		if err != nil {
			log.Info(fmt.Sprintf("couldn't get file path for '%s': %v", k, err))
			continue
		}
		if err := os.Remove(fp); err != nil {
			log.Info(fmt.Sprintf("couldn't remove file '%s': %v", fp, err))
			continue
		}
	}
...
func PruneLayers() error {
	...
	if err != nil {
		log.Info(fmt.Sprintf("couldn't read dir '%s': %v", p, err))
		return err
	}
		...
		if err != nil {
			if errors.Is(err, ErrInvalidDigestFormat) {
				...
				if err := os.Remove(filepath.Join(p, blob.Name())); err != nil {
					log.Error("couldn't remove blob", "blob", blob.Name(), "error", err)
				}
			}
	...
	log.Info(fmt.Sprintf("total blobs: %d", len(deleteMap)))

	if err := deleteUnusedLayers(deleteMap); err != nil {
		log.Error(fmt.Sprintf("couldn't remove unused layers: %v", err))
		return nil
	}

	log.Info(fmt.Sprintf("total unused blobs removed: %d", len(deleteMap)))
...
func PushModel(ctx context.Context, name string, regOpts *RegistryOptions, fn func(api.ProgressResponse)) error {
	...
	for _, layer := range layers {
		if err := uploadBlob(ctx, mp, layer, regOpts, fn); err != nil {
			log.Info(fmt.Sprintf("error uploading blob: %v", err))
			return err
		}
	}
...
func PullModel(ctx context.Context, name string, regOpts *RegistryOptions, fn func(api.ProgressResponse)) error {
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		log.Warn("pulling model with bad existing manifest", "name", name, "error", err)
	} else {
		...
		if err := verifyBlob(layer.Digest); err != nil {
			if errors.Is(err, errDigestMismatch) {
				...
				if err := os.Remove(fp); err != nil {
					...
					log.Info(fmt.Sprintf("couldn't remove file with digest mismatch '%s': %v", fp, err))
				}
			}
			return err
		}
	...
	if err != nil {
		log.Info(fmt.Sprintf("couldn't write to %s", fp))
		return err
	}

	if !config.Conf.NoPrune && len(deleteMap) > 0 {
	...
	for range 2 {
		...
		if err != nil {
			if !errors.Is(err, context.Canceled) {
				log.Info(fmt.Sprintf("request failed: %v", err))
			}
...
func makeRequest(ctx context.Context, method string, requestURL *url.URL, headers http.Header, body io.Reader, regOpts *RegistryOptions) (*http.Response, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("llama.go/%s (%s %s) Go/%s", version.String(), runtime.GOARCH, runtime.GOOS, runtime.Version()))
